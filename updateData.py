...
yearHighList = pd.DataFrame(columns=['Stock','Date','Open','High','Low','Close','Volume','52weeksHigh', '3monthsHigh','averageVol','3monthlow'])
for fileName in fileList:
    ...
    for stock in stockList:
        ...
        checkStock["3monthsHigh"] = checkStock.High.rolling(window=65, min_periods=1, center=False).max()
        checkStock["averageVol"] = checkStock.Volume.rolling(window=10, min_periods=1, center=False).mean()
        checkStock["3monthlow"] = checkStock.Volume.rolling(window=60, min_periods=1, center=False).min()
