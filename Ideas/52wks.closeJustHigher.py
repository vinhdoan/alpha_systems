...
for fileName in fileList:
    ...
    if len(checkStock>252):
        ...
        while i < len(checkStock)-10:
            if openingOrder==0:
                if  checkStock.ix[i]['Close']>= checkStock.ix[i]['Open'] >= checkStock.ix[i-1]['High']  and (checkStock.ix[i]['High'] > checkStock.ix[i-1]['52weeksHigh']) and checkStock.ix[i]['Close']> checkStock.ix[i-1]['High'] *1.03 and checkStock.ix[i-1]['averageVol']>50000:
...
orders.to_csv('testresults/52wksbreakout-buySellClose-15percent-openHigherThanLastHigh.csv',index=False)
