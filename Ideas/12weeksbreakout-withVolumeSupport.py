...
for fileName in fileList:
    ...
    checkStock["12weeksHigh"] = checkStock.High.rolling(window=60,min_periods=1,center=False).max()
    ...
    if len(checkStock>252):
        ...
        while i < len(checkStock)-10:
            if openingOrder==0:
                if (checkStock.ix[i]['High'] > checkStock.ix[i-1]['12weeksHigh']) and  checkStock.ix[i]['High']> checkStock.ix[i-1]['High'] * 1.03  and  checkStock.ix[i-1]['averageVol']>100000:
                    lastHigh = checkStock.ix[i-1]['12weeksHigh']
...
orders.to_csv('testresults/12wksbreakout-buyatNextDay-15percent-withoutGap-buyatClose.csv',index=False)
statistics.to_csv('testresults/12wksbreakout-buyatNextDay-15percent-withoutGap-buyatClose-stats.csv',index=False)
