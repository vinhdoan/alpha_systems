...
for fileName in fileList:
    ...
    checkStock["52weeksHigh"] = checkStock.High.rolling(window=126,min_periods=1,center=False).max()
    ...
    if len(checkStock>252):
        ...
        while i < len(checkStock)-10:
            if openingOrder==0:
                if  checkStock.ix[i]['Close']>= checkStock.ix[i]['Open'] and (checkStock.ix[i]['High'] >= checkStock.ix[i-1]['52weeksHigh']*1.03) and checkStock.ix[i-1]['averageVol']>30000:
...
orders.to_csv('testresults/26wks-withgap.csv',index=False)
statistics.to_csv('testresults/26wks-withgap-stats.csv',index=False)
