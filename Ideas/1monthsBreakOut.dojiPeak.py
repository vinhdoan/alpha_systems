...
for fileName in fileList:
    ...
    checkStock["52weeksHigh"] = checkStock.High.rolling(window=25,min_periods=1,center=False).max()
            ...
            if openingOrder==0:
                if  (checkStock.ix[i]['High'] > checkStock.ix[i-1]['52weeksHigh']) and checkStock.ix[i]['High']> checkStock.ix[i-1]['High'] *1.03 and checkStock.ix[i]['High']< checkStock.ix[i-2]['High'] *1.1 and checkStock.ix[i-1]['averageVol']>50000:
                    ...
                    for j in range(10,25):
                        if checkStock.ix[i-j]['High'] == lastHigh and (checkStock.ix[i-j]['High'] > checkStock.ix[i-j]['Low']*1.03) and (checkStock.ix[i-j]['Close']==checkStock.ix[i-j]['Low']):
...
orders.to_csv('testresults/1mths-breakout-lastDoji.csv',index=False)
