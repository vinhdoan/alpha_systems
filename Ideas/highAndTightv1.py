    ...
    for fileName in fileList:
        ...
        if len(checkStock)>252:
            ...
            while i < len(checkStock)-25:
                if checkStock.ix[i]['High'] >= 1.6 * checkStock.ix[i-1]['2monthsLow'] and checkStock.ix[i-1]["averageVol"] > 20000 and checkStock.ix[i]['High'] >checkStock.ix[i+1]['Close'] and checkStock.ix[i]['High'] >checkStock.ix[i+2]['Close']  :
                    newOrder =  pd.DataFrame([[fileName,checkStock.index[i]]],columns=['stock', 'buyDate'])
                    orders = pd.concat([orders, newOrder])
                i+=20
    orders.to_csv('testresults/flag-high-and-tightv1.csv')
