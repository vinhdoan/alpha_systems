...
for fileName in fileList:
    ...
    checkStock['2weeksHigh'] = checkStock.High.rolling(window=10,min_periods=1,center=False).max()
    if len(checkStock>252):
        ...
        while i < len(checkStock)-10:
            if openingOrder==0:
                if (checkStock.ix[i]['High'] > checkStock.ix[i-1]['52weeksHigh']) and checkStock.ix[i]['Close']> checkStock.ix[i-1]['High'] *1.03 and checkStock.ix[i-1]['2weeksHigh']< checkStock.ix[i-1]['52weeksHigh'] and checkStock.ix[i-1]['averageVol']>100000:
...
orders.to_csv('testresults/52wksbreakout-buySellClose-15percent-justBreak.csv',index=False)
# statistics.to_csv('testresults/52wksbreakout-buySellClose-15percent-withoutGap-buyatClose-volGreater5Bil-stats.csv',index=False)
