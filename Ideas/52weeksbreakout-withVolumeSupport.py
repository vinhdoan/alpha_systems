...
for fileName in fileList:
    ...
    if len(checkStock>252) and not 'ABT' in fileName:
        ...
        while i < len(checkStock)-10:
            if openingOrder==0:
                if (checkStock.ix[i]['High'] > checkStock.ix[i-1]['52weeksHigh']) and checkStock.ix[i]['High']> checkStock.ix[i-1]['High'] * 1.03  and  checkStock.ix[i-1]['averageVol']>100000:
...
orders.to_csv('testresults/52wksbreakout-buyatClose-withStdev.csv',index=False)
statistics.to_csv('testresults/52wksbreakout-buyatClose-withStdev-stats.csv',index=False)
