...
#list margin
kisMargin = pd.read_csv('margin-kis.csv')
fptMargin = pd.read_csv('margin-fpt.csv')
            ...
            if result.status_code==200:
                ...
                if len(symbol)==3:
                    ...
                    if(len(data)>0):
                        ...
                        if last > df.ix[i]['52weeksHigh'] and highest >= df.ix[i]['High']*1.025 and int(df.ix[i]['averageVol']>100000): #open >= df.ix[i]['High'] and
                            vol1 = (int)(kis/last)
                            vol2 = (int)(fpt/last)
                            mgK = "no"
                            mgF = "no"
                            ...
                            slackdata = {'text': "mua được:" + symbol+ " - " + '%.2f' % last + ' tối đa ' + str(int(df.ix[i]['averageVol']/20)) + " cổ phiếu" + info+ '\n'+'---------------'}
